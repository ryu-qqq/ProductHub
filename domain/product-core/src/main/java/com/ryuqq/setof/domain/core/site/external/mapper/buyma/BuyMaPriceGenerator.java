...
public class BuyMaPriceGenerator {
     ...
     * 최종 가격을 계산합니다.
     *
     * @param price 세토프 판매가 (현재 가격 및 정가 포함).
     * @param exchangeRate 환율 (엔/원).
     * @return 최종 외부 몰 가격 (배송비 포함).
     */
    public BigDecimal calculateFinalPrice(Price price, BigDecimal exchangeRate) {
        ...
        BigDecimal basePrice = calculateBasePrice(price.getCurrentPrice(), normalizedExchangeRate);
        ...
        BigDecimal finalPrice = baseWithShipping.compareTo(BigDecimal.valueOf(16666)) > 0
                ? calculateAdjustedPrice(price.getCurrentPrice(), normalizedExchangeRate)
                : basePrice;
