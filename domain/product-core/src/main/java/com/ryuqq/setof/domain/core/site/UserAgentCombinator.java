...
public class UserAgentCombinator {
    ...
    public Map<String, String> generateRandomHeaders(String configurationName) {
        ...
        List<HeaderConfigurationDto> configurations = headerConfigurationQueryDslRepository.fetchConfigurationsByName(configurationName);
        configurations.forEach(config -> headers.put(config.getHeaderName(), config.getHeaderValue()));

        List<HeaderDto> randomHeaders = headerQueryDslRepository.fetchMultipleRandomHeaders(5);
